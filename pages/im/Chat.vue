      ...
      <div class="window-and-input">
        <div class="header" style="padding: 5px; line-height: 30px">
          <a-space v-if="friend">
            ...
            <span style="font-weight: 600; font-size: 16px; color: #787878">{{
              friend.name
            }}</span>
          </a-space>
        </div>
        <div class="window" ref="msgWindow">
          <template v-for="(context, index) in chatMessage">
            <p
              v-if="context.type === 'send'"
              :key="index"
              class="msg-layout msg-layout-right"
            >
              <a-space>
                <span class="msg-bubble msg-bubble-right">{{
            ...
            <p
              v-if="context.type === 'reverse'"
              :key="index"
              class="msg-layout msg-layout-left"
            >
              <a-space>
                <a-avatar :src="context.data.avatar" />
                <span class="msg-bubble msg-bubble-left">{{
                  context.data.content
                }}</span>
              </a-space>
            </p>
        ...
        <div
          class="tool"
          style="padding: 8px 10px; box-sizing: border-box; font-size: 18px"
        >
          <a-space>
            <a href="#" @click="() => {}">
...
回车发送</pre
      ...
      <div class="list-and-tools">
          ...
          <a-list item-layout="horizontal" :split="false" :loading="loading">
            <template v-for="item in userItems">
              <a-list-item :key="item.id">
                <a @click="beginSession(item)" black>
                <a-list-item-meta>
                  <template slot="title">
                   <span>{{ item.name }}</span>
                  </template>
        ...
        <a-card title="个人信息" :bordered="false" size="small">
          <template #extra>
            <a-button type="link" @click="exit">退出</a-button>
          </template>
...
export default {
  ...
  methods: {
    createConnection() {
      this.socket = new WebSocket("ws://43.142.114.37/ws?" + this.self.id);
    ...
    onmessage(event) {
      ...
      if (data.content) {
        ...
        this.chatMessage.push({
          type: "reverse",
          data: { ...data, avatar: this.friend.avatar },
        });
        this.$nextTick(() => {
          this.$refs["msgWindow"].scrollTop =
            this.$refs["msgWindow"].scrollHeight + 300;
        });
      }
    },
    send() {
      ...
      let data = {
        command: "one.one.chat",
        data: {
          target: this.friend.id,
          source: this.self.id,
      ...
      this.chatMessage.push({
        type: "send",
        data: { ...data.data, avatar: this.self.avatar },
      });
      this.inputMsg = null;
      this.$nextTick(() => {
        this.$refs["msgWindow"].scrollTop =
          this.$refs["msgWindow"].scrollHeight + 50;
      });
    },
    beginSession(item) {
      this.chatMessage = [];
...
.msg-bubble {
  ...
  max-width: 300px;
  vertical-align: top;
  box-shadow: 1px 1px 40px rgba(37, 11, 11, 0.3);
}
.msg-bubble-right {
